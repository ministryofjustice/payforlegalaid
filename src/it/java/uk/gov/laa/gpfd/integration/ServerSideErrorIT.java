...
class ServerSideErrorIT extends BaseIT {
    ...
    void setUpMojfinDatabase() {
        ...
        // Manually create the metadata tables in PostgreSQL if they don't exist
        try {
            // Create glad schema
            ...
            // Verify tables exist
            var tableCount = metadataJdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'glad'", Integer.class);
            System.err.println("=== Total tables in glad schema: " + tableCount);
            
            var reportCount = metadataJdbcTemplate.queryForObject("SELECT COUNT(*) FROM glad.reports", Integer.class);
            System.err.println("=== Reports count: " + reportCount);
    ...
    void getReportsShouldNotDependOnMojfinGpfdMetadataTables() throws Exception {
        ...
        Assertions.assertEquals(200, result.getResponse().getStatus(), 
            "Expected 200 but got " + result.getResponse().getStatus() + ": " + result.getResponse().getContentAsString());
    }

    @Test
    void getReportByIdShouldNotDependOnMojfinGpfdMetadataTables() throws Exception {
        MvcResult result = performGetRequestWithRoles("/reports/b36f9bbb-1178-432c-8f99-8090e285f2d3", List.of("Financial"))
                .andReturn();
