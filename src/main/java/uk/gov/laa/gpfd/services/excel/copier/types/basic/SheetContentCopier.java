...
public abstract class SheetContentCopier {
  ...
  /**
   * Creates a {@link BiConsumer} that copies cell content (values, styles) from
   * source to target sheet.
   *
   * @formatting reference to underlying formatting.
   */
  public static BiConsumer<Sheet, Sheet> createContentCopier(Formatting formatting) {
    return new ContentCopier(formatting);
  }

  /**
   * Creates a {@link BiConsumer} that copies column widths from source to target
   * sheet.
   */
  ...
  /**
   * Creates a {@link BiConsumer} that copies merged regions from source to target
   * sheet.
   */
   ...
   */
  public static BiConsumer<Sheet, Sheet> copyAll(Formatting f) {
    return createContentCopier(f)
            .andThen(createMergedRegionCopier())
            .andThen(createColumnWidthCopier());
  }
  ...
  static class ContentCopier implements BiConsumer<Sheet, Sheet> {
    ...
    private final Formatting styleManager;

    ContentCopier(Formatting styleManager) {
