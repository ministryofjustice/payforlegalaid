...
-- (one per Excel tab, for example). report_id is a FK to reports.
-- The "index" column controls tab ordering; quoted because INDEX is reserved in some SQL dialects.
...
-- field_attributes
-- Column-level metadata for a report query: display name, format, width, order.
-- report_query_id is a FK to report_queries.
...
-- role_id is a small integer, not a UUID, matching the Oracle source.
